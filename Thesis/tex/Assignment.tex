...
The aforementioned equiplets needs to work together to create products constructed of various product steps. In order to let these equiplets work together, they are placed in a so called grid. Figure \ref{gridLayout} shows an example of this grid with nine equiplets. These equiplets are placed in a square with space inbetween the equiplets. The space allows products to travel from equiplet to equiplet. In future REXOS this grid has to be able to add and/or remove equiplets without shutdown of the system.
