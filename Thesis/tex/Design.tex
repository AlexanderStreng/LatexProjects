...
\section{Simulation}
Collecting performance data on the REXOS platform was neccessary, however, running sets and sets of test cases on the equiplets would be very time consuming. For this reason, the decision was made to develop a simulator for the platform. This simulator would be capable of running test cases without requiring the use of the hardware.

The simulator is focused around MAS. Simplified versions of the agents were written to emulate Jade within the simulation. These agents perform every action the original agents do, and communicate with each other as the original agents do, the communication is however, not compliant with the FIPA\footnote{http://www.fipa.org/} Standard. ROS was not simulated completely, in more detail: no hardware actions are simulated, there is however, an equiplet. This equiplet has the possiblity of going into an error state.
