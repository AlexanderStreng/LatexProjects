...
In this section the main defects from the current implementation of the REXOS architecture are analysed and new design implementations are proposed.

\subsection{Agent communication}
\label{sec:AgentComm}
On the MAS side of the system, certain things needed to change. One of the biggest problems was getting a proper overview on how the system functioned and how communication was implemented. After some research figure \ref{AgentCommunicationOverview} was drawn.
\begin{center}
...
\end{center}
Marked in red are the changes that need to be made to make all communication FIPA compliant\footnote{http://www.fipa.org/specs/fipa00086/XC00086D.pdf}. This has to be done in order to have a coherent system.

\subsection{Knowledge database}
\label{sec:kdb}
% was done for autonomous 
The knowledge database was initially designed for a centralized platform. During one of the brainstorms sessions that took place, a decision was made to make equiplets autonomous. To accomodate this change, the knowledge database required re-designing. The task of designing the new database was assigned to one of the interns, T. Bakker. The resulting new design incoporated module location and calibration data for the equiplet modules. The full design can be found in figure~\ref{fig:kdb}.
...
The scheduling algorithm that was present in REXOS did by far not suffice for optimizing scheduling within the grid. Because a new scheduling algorithm was needed, research was conducted on implementing the algorithm from Moergestel's paper \emph{‘Multiagent-based agile manufacturing: from user requirements to product.}\cite{PAPER70} A few changes to the implementation of this algorithm had to be made. In this section the implementation of the algorithm in REXOS is explained.
...
(steps Ø 3,Ø 4 \& Ø 5 on E1 are multiplied by (1 - 0.6) which results in 3 $\times$ 0.4)
...
Using this approach has a disadvantage when used in an agile manufacturing environment and will be discussed below.
...
In real life situations an equiplet operates from product step to product step. When an equiplet does not have a product step planned, it doesnt have to do anything special. Translating this to software means that having a linked list of product steps is enough to let the equiplet function. The issue with using a circular buffer is that all timeslots will be stored regardless of whether they are empty or not. If the equiplet needs to execute 20\% of the time, would mean that that 80\% of the timeslots in the circular buffer would be empty. This is unneccessary used memory. A linked list will resolve this issue. It would make the schedule of an equiplet more scalable and less memory heavy.
...
To solve this problem, this situation has to be detected and resolved. Detection can be done as following: When an product agent (PA$_1$) cannot get the lock of an equiplet's schedule (EQ$_3$), it will request the address of the product agent that locked EQ$_3$, resulting in the address of PA$_2$. PA$_1$ will then send a message to PA$_2$, notifying PA$_2$ that PA$_1$ is in need of the locked EQ$_3$. When PA$_2$  will detect a locked EQ$_1$ that is locked by PA$_1$, by asking EQ$_1$ the address of the locker, PA$_2$ will compare the address returned with its notification list and will detect the deadlock of two product agents that need each others locked equiplets.
...
\label{EquipletError}
In real life situations, the hardware or software of equiplets can fail at any given time. There are two kinds of errors, hardware errors and software errors. Software errors could occur when runtime undefined exceptions could occur. When the software cannot handle this exception, it will be put into an error state. Hardware errors will occur when the hardware does not act expected. When the software detects this deviant behaviour of the hardware and cannot resolve it, the software will also put the equiplet into an error state.
...
\section{Simulation}
Collecting performance data on the REXOS platform was neccessary, however, running extensive sets of test cases on the equiplets nearly impossible. For this reason, the decision was made to develop a simulator that simulates the cognitive part of REXOS. This simulator would be capable of running test cases without requiring the equiplets themselves. The simulater could then also process a said timeframe as fast as possible.

The simulator is focused around MAS. To simulate the entities in the MAS part of REXOS, simplified versions of the agents have to be written. These entities would be classes that would have the same features as the agents. The communication part between agents has been removed by doing this; method calls are used to simulate this. Using ACL messages of the jade platform would be slow to use in a simulation.

The hardware execution parts in REXOS has been implemented in te simulation, but limited. Three parts will be implemented: capabilities and hardware actions and equiplet errors. Capabilities will be available just as in the REXOS variant. Hardware actions will be simulated as if they will be executed correctly every time. Finally, equiplet errors have been added. When an equiplet is being put in error, products can no longer be produced there. The design in section \ref{EquipletError}.

\subsection{Mechanics}
The simulator that was developed uses a concept typically found in games, a single threaded update loop in which the simulation is updated in each iteration of that loop. A list of updateables is tracked and these are updated during each iteration, or tick, of the simulation. This would however limit the ability to simulate the lock design described in section \ref{sec:schedulelock}. Instead, the lock action is always being accepted since there is only one thread.

The simulator has a graphical user interface which runs on it's own thread, this to ensure that the simulation is not interrupted, and also to be able to keep the user interface updated with live feedback during the simulation.

\subsubsection*{Scheduling}
The scheduling algorithm implemented into the simulation was developed during the research semester. Although the algorithm was partially implemented into the REXOS platform, it was fully implemented in the simulation. By running test cases, the algorithm can be tested. More information on the algorithm itself can be found in section ~\ref{sec:Scheduling} on page~\pageref{sec:Scheduling}.

\subsubsection*{Data Acquisition}
Data is gathered using data collectors. These collectors have a collectData method, which is specific for the type of data being collected. There is a data collector for equiplets and one for products. For each update cycle, the collector checks whether it is time to gather new data, in the case that new data needs to be collected, it attempts to process its specific type of updateables.
...
The editor interface will handle the easy creation and modification of the configuration files. For this reason, the interface will have the capability of creating virtual representations of capabilities, products, batches, equiplets and the grid. The editor interface will have the ability to output the created configuration to the seperate files.
