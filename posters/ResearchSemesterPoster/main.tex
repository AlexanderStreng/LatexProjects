...
\newcommand{\threecolumnsblock}[4][Title]{
    \columnblock[#1]{
        \begin{multicols}{2}
            #2
            \newpage
...
Manufacturing paradigms such as Agile Manufacturing (AM) and Reconfigurable Machine Systems (RMS) revolve around flexibility. The REXOS project utilizes a Grid Manufacturing setup, which is based on RMS. In Grid Manufacturing, different approaches can be used for scheduling, namely hierarchical and heterarchical. This poster compares the two approaches using a simulation program. And explores the pros and cons considering both setups.
    ...
    \begin{columns}[t]   
		\begin{column}{.48\textwidth}
			...
			In this setup a hierarchical grid is displayed.
    }
...
As seen in the results, heterarchical scheduling can produce more per timeframe. Hierarchical scheduling has lower throughput, this can be linked to the fact the current load of the equiplets in a hierarchical setup.

%Throughput (2)
When using a hierarchical setup, products and batch products will be placed in a more controlled situation and thus will have lower transportation needs. Heterarchical will have higher transportation needs since the maximal transportation distance is higher.

%FAILED Products (3)
An heterarchical setup tends to be more 'chaotic'. Production is faster, but products will be longer in the grid with a higher chance of failing products. These failing products have failed because their deadline has been expired, which can result because of long transport times.
				}   
   ...
   \begin{columns}[t]  
			\begin{column}{.65\textwidth}
							...
							\justifying 
Using a heterarchical schedulng setup, equiplets will have a higher throughput, but at the cost of transportation higher costs. Giving the grid a high load could also result in more failed products. This setup can be desired when there are only a lot of varying products that need to be produced at the same time.
Hierarchical scheduling is recommended when there are limited transportation resources or if there is a need to control production. Reserving equiplets for batches of products can be desired to be sure the batch will be finished in time.
				}
            \end{column}
